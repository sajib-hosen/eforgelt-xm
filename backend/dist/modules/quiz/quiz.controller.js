...
exports.getCurrentStep = exports.submitQuizAnswers = void 0;
const evaluate_quiz_1 = require("../../utils/evaluate-quiz");
...
const submitQuizAnswers = async (req, res) => {
    try {
        ...
        let previousResult;
        if (user.id) {
            previousResult = await (0, quiz_service_1.getQuizResult)({ userId: user.id });
        }
        else if (user.email) {
            previousResult = await (0, quiz_service_1.getQuizResult)({ email: user.email });
        }
        else {
        ...
        // Evaluate answers
        const result = (0, evaluate_quiz_1.evaluateQuiz)(data, step);
        let saveRes;
        if (previousResult && step === previousResult.step + 1) {
            // New step after previous - create a new result
            saveRes = await (0, quiz_service_1.saveQuizResult)({
                userId: user.id,
                email: user.email,
                step,
                answers: data,
            ...
            saveRes = await (0, quiz_service_1.updateQuizResult)({
                userId: user.id,
                email: user.email,
                step,
                answers: data,
...
const getCurrentStep = async (req, res) => {
    try {
        ...
        const previousResult = await (0, quiz_service_1.getQuizResult)({
            userId: user.id,
            email: user.email,
        });
        if (!previousResult) {
        ...
        console.error("getCurrentStep error:", error);
        return res.status(500).json({ message: "Internal server error" });
    }
};
